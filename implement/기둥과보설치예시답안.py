...
def possible(answer):
    for x, y, itme in answer:
        # 기둥일 경우 바닥에 있거나 보위에 있거나 기둥위에 있다면 패스 아니면 False return
        if itme == 0:
            if y == 0 or [x-1, y, 1] in answer or [x, y, 1] in answer or [x, y-1, 0] in answer:
                continue
            return False
            # 보일 경우, 양 끝중 하나에 기둥이 있거나 양쪽에 보가있다면 패스 아니면 False return
        elif itme == 1:
...
def solution(n, build_frame):
    ...
    for frame in build_frame:
        x, y, item, operate = frame
        # 삭제하는 동작일 경우 일단 삭제후 answer 전체를 확인하여 불가능하다면 다시 추가
        if operate == 0:
            answer.remove([x, y, item])
            if not possible(answer):
                answer.append([x, y, item])
        # 추가하는 동작일 경우, 일단 추가 후, answer 전체를 확인하여 불가능하다면 다시 제거
        if operate == 1:
            answer.append([x, y, item])
            if not possible(answer):
                answer.remove([x, y, item])
